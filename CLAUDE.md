...
| `src/presentation/components/EarlyPayoffSimulator.tsx`                | Ephemeral per-installment "quitação antecipada" what-if calculator — see "Purchase planning" below |
...
5. `EarlyPayoffSimulator.tsx` is a separate, ephemeral (not persisted) what-if tool: per installment, the user picks a competência they actually intend to pay it in, and `presentValueDiscount()` (`amortization.ts`) discounts the nominal installment at the contract's own monthly rate for however many months early that is — the "quitação antecipada" discount Brazilian lenders owe under CDC art. 52 §2º. Validated against a real CDC vehicle-financing payoff statement (paying ~43 months early discounted R$ 989,70 to R$ 591,78); the app's month-only `competencia` granularity (vs. the real contract's exact days) means results land within a few reais of a real statement, not to the cent.
