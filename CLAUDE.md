...
- **`application/repositoryProvider.ts`** is the single decision point: it builds a `GoogleSheetsRepository` for the current user's spreadsheet (resolved via `spreadsheetStore`, keyed by email) and caches the instance per spreadsheet id. The first time it builds a repo for a spreadsheet in a session, it also fires `SheetsInitializer.ensureSheets()` in the background (fire-and-forget, errors swallowed) — this is the app's **schema self-healing**: any tab/column a shipped feature needs (a new sheet, a new column like `Transaction.plan_id`) gets created on an already-onboarded user's spreadsheet automatically, without asking them to revisit `/setup` or edit their Sheet by hand. This matters because the app is in production with real users on their own spreadsheets — see "Google Sheets schema" below for what still can't be automated (data already in old rows, obviously, and any *rename/removal* of a column, which `ensureSheets` doesn't attempt).
...
| `purchase_plans`      | `plan_id, nome, descricao, valor_compra, valor_entrada, taxa_juros, taxa_juros_periodicidade, numero_parcelas, forma_amortizacao, competencia_inicio, margem_minima, categoria_id, payment_account_id, status, created_at, updated_at` |

If you add a field to a domain type, add it here to `SHEET_SPECS` (`SheetsInitializer.ts`) — that's the only place that needs updating. New tabs and new columns on existing tabs then reach every already-onboarded user's spreadsheet automatically via the schema self-healing described above (`repositoryProvider.ts` fires `ensureSheets()` once per session); there is no manual step for the user anymore. What self-healing does **not** do: backfill a value into the new column for rows that existed before it (they just read as empty/`null`, which every reader already treats as "unset"), or rename/remove a column (`ensureSheets` only adds what's missing and rewrites the canonical header row — it never deletes a column, even one no longer in `SHEET_SPECS`).
...
`PurchasePlan` (`purchase_plans` sheet) is a saved simulation for a large financed purchase (e.g. a car): full price (`valor_compra`) minus an optional down payment (`valor_entrada`) is what actually gets financed (`financedAmount()` in `purchasePlanning.ts`), plus interest rate + periodicity, number of installments, amortization method, desired start competência, and a minimum monthly margin the user wants to keep. Its installment schedule is **never persisted** — it is 100% derivable from those fields and recomputed client-side on demand:

1. `src/lib/amortization.ts` builds the raw schedule (`buildPriceTable`/`buildSacTable`/`buildLinearTable`, dispatched by `buildAmortizationTable`) from the **financed amount** (already net of `valor_entrada`) — pure math, no domain types. `PRICE` is a fixed installment with decreasing interest; `SAC` is constant amortization with a decreasing installment; `SEM_JUROS` is the linear no-interest split (same math as `TransactionDialog`'s simple installment flow), kept as a baseline to compare "financiar com juros" vs "parcelar sem juros". Real financing contracts (car/imóvel) often bundle IOF/seguro/tarifas into the financed amount and separate the nominal rate from the CET — for the simulation to match a real proposal, `valor_compra`/`valor_entrada` should net out to that contract's actual financed principal, and `taxa_juros` should be the nominal rate, not the CET. There is no support for index-linked correction (TR) or graduated/interest-only "fase de obra" schedules (real estate financing during construction) — those are out of scope.
...
5. Each row in the Parcelas table (`AmortizationTable.tsx`) has an editable competência picker, ephemeral/not persisted (local `payoffDates` state in `PurchasePlanDetailPage`) — when do you actually intend to pay that installment, defaulting to its due date. Paying ahead of schedule discounts the value via `presentValueDiscount()` (`amortization.ts`) at the contract's own monthly rate for however many months early that is — the "quitação antecipada" discount Brazilian lenders owe under CDC art. 52 §2º. The verdict icon on each row still reflects the *original* due month's budget fit, independent of the chosen payment date. Validated against a real CDC vehicle-financing payoff statement (paying ~43 months early discounted R$ 989,70 to R$ 591,78); the app's month-only `competencia` granularity (vs. the real contract's exact days) means results land within a few reais of a real statement, not to the cent.
