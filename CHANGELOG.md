...
- Purchase planning: the Parcelas table's competência per row is now an
  editable picker — when do you actually intend to pay that installment,
  defaulting to its due date — showing the present-value discount for
  paying ahead of the due date at the contract's own monthly rate (the
  "quitação antecipada" discount Brazilian lenders owe under CDC art. 52
  §2º), plus a running total and total interest saved across the whole
  schedule. Validated against a real CDC
